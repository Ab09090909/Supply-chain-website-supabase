...
def _get_writer_client():
    """Get the best client for write operations.

    Uses the admin (service_role) client by default — this bypasses
    RLS so orders always go through. The user_id is taken from the
    server-side session, not from any client input, so there's no
    privilege escalation risk.
    """
    try:
        from database.connection import get_supabase_admin_client
        return get_supabase_admin_client(), True
    except Exception:
        return get_supabase_client(), False
...
def render_customer_cart():
    ...
    with col2:
        ...
        if st.button("Place Order", type="primary", use_container_width=True):
            _place_order(user, cart_items, subtotal, tax, shipping, total)


def _place_order(user, cart_items, subtotal, tax, shipping, total) -> None:
    """Place the order using the admin client to bypass RLS."""
    if not user.get("id"):
        st.error("Your user profile is missing. Please log in again.")
        return
    ...
    # Use the admin client to bypass RLS entirely
    writer, _ = _get_writer_client()

    try:
        insert_response = writer.table("orders").insert(order_payload).execute()
    ...
    if new_order and new_order.get("id"):
        ...
        try:
            writer.table("order_items").insert(items_to_insert).execute()
        except Exception as items_err:
            # Roll back the half-created order
            try:
                writer.table("orders").delete().eq("id", new_order["id"]).execute()
            except Exception:
                pass
            st.error(f"Order could not be saved completely. Rolled back. Please try again. Detail: {items_err}")
            return

    # Clear the cart
    try:
        writer.table("cart_items").delete().eq("user_id", user["id"]).execute()
    except Exception:
        pass
